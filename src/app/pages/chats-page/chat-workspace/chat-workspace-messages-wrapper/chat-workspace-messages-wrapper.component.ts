import {AfterViewInit, Component, ElementRef, inject, input, OnDestroy, OnInit, Renderer2, signal} from '@angular/core';
...
import {BehaviorSubject, debounceTime, firstValueFrom, fromEvent, map, Subscription, tap} from 'rxjs';
...
messagesStart.forEach(message => {
  const messageDate = DateTime.fromISO(message.createdAt, {zone: 'utc'})
    .setZone(DateTime.local().zone).startOf('day');
